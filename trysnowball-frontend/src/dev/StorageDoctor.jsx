...
function detectPoundsFromLegacy(value) {
  ...
  if (value >= 100000 && value % 100 === 0) return value / 100; // pence → pounds
...
export default function StorageDoctor() {
  ...
  const computeIdbTotals = (snap) => {
    ...
    const minCents = debts.reduce(
      (s, d) => s + (d.min_payment_cents ?? Math.round((d.minPayment ?? 0) * 100)),
      0
    );
    ...
    return {
      ...
      debtsTable: debts.map((d) => ({
        ...
        apr: d.apr_bps != null ? (d.apr_bps / 100).toFixed(2) : (d.interest ?? 0),
        min: ((d.min_payment_cents ?? Math.round((d.minPayment ?? 0) * 100)) / 100).toFixed(2),
  ...
  const refreshIdb = async () => {
    try {
      ...
      setIdbSnapshot(snap);
      setShowIdb(true);
      setMsg("📸 IndexedDB snapshot captured.");
    } catch (e) {
      console.error(e);
      setMsg("❌ Couldn’t read IndexedDB.");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="p-4 mb-4 rounded-lg border bg-amber-50">
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="text-xs text-amber-700">{busy ? "Working…" : null}</div>
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        <button
          ...
          onClick={run(async () => {
            ...
            setIdbSnapshot(null);
            setMsg("🧨 Cleared IndexedDB and legacy localStorage.");
          })}
