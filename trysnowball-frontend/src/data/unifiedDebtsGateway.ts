...
export async function fetchAllDebts(): Promise<UKDebt[]> {
  console.log('🏴󠁧󠁢󠁥󠁮󠁧󠁿 [UNIFIED_GATEWAY] Fetching all debts');
  ...
  console.log('🏴󠁧󠁢󠁥󠁮󠁧󠁿 [UNIFIED_GATEWAY] Fetched', data.debts.length, 'debts');
...
function sanitizeDebtForD1(debt: any): any {
  ...
  for (const [key, value] of Object.entries(debt)) {
    ...
    if (value !== undefined && value !== null && value !== '') {
      // Convert string numbers to actual numbers for UK format fields
      if (['amount', 'min_payment', 'apr', 'order_index', 'limit', 'original_amount'].includes(key)) {
  ...
  console.log('🔧 [SANITIZE] Input:', debt);
  console.log('🔧 [SANITIZE] Output:', sanitized);
...
export async function createDebt(debt: UKDebt): Promise<UKDebt> {
  console.log('🏴󠁧󠁢󠁥󠁮󠁧󠁿 [UNIFIED_GATEWAY] Creating debt:', debt.name);
  
  // Sanitize debt to remove undefined values (D1 doesn't accept them)
  const sanitizedDebt = sanitizeDebtForD1(debt);
  console.log('🏴󠁧󠁢󠁥󠁮󠁧󠁿 [UNIFIED_GATEWAY] Sanitized debt for D1:', sanitizedDebt);
  ...
  const data = await response.json();
  console.log('🏴󠁧󠁢󠁥󠁮󠁧󠁿 [UNIFIED_GATEWAY] ✅ Debt created:', data.debt.id);
...
export async function updateDebt(debtId: string, updates: Partial<UKDebt>): Promise<void> {
  console.log('🏴󠁧󠁢󠁥󠁮󠁧󠁿 [UNIFIED_GATEWAY] Updating debt:', debtId);
  
  // Sanitize updates to remove undefined values (D1 doesn't accept them)
  console.log('🏴󠁧󠁢󠁥󠁮󠁧󠁿 [UNIFIED_GATEWAY] Raw updates before sanitization:', updates);
  const sanitizedUpdates = sanitizeDebtForD1(updates);
  console.log('🏴󠁧󠁢󠁥󠁮󠁧󠁿 [UNIFIED_GATEWAY] Sanitized updates for D1:', sanitizedUpdates);
  
  // Check if any values are still undefined
  for (const [key, value] of Object.entries(sanitizedUpdates)) {
    if (value === undefined) {
      console.error(`🚨 [UNIFIED_GATEWAY] UNDEFINED VALUE FOUND: ${key} = ${value}`);
    }
  }
  ...
  const safeBody = JSON.stringify(sanitizedUpdates, (key, value) => {
    if (value === undefined) {
      console.error(`🚨 Found undefined in JSON.stringify: ${key}`);
      return null; // Convert undefined to null for D1
    }
    return value;
  });
  
  console.log('📤 [UNIFIED_GATEWAY] Final JSON being sent:', safeBody);
  ...
  console.log('🏴󠁧󠁢󠁥󠁮󠁧󠁿 [UNIFIED_GATEWAY] ✅ Debt updated:', debtId);
...
export async function deleteDebt(debtId: string): Promise<void> {
  console.log('🏴󠁧󠁢󠁥󠁮󠁧󠁿 [UNIFIED_GATEWAY] Deleting debt:', debtId);
  
  await fetchWithAuth(RouteHelpers.getRoute('debts.delete', debtId), {
    method: 'DELETE'
  });

  console.log('🏴󠁧󠁢󠁥󠁮󠁧󠁿 [UNIFIED_GATEWAY] ✅ Debt deleted:', debtId);
...
export async function batchCreateDebts(debts: UKDebt[]): Promise<UKDebt[]> {
  console.log('🏴󠁧󠁢󠁥󠁮󠁧󠁿 [UNIFIED_GATEWAY] Batch creating', debts.length, 'debts');
  ...
  console.log('🏴󠁧󠁢󠁥󠁮󠁧󠁿 [UNIFIED_GATEWAY] ✅ Batch created', results.length, 'debts');
  return results;
}
...
export async function healthCheck(): Promise<boolean> {
  try {
    ...
    console.warn('🏴󠁧󠁢󠁥󠁮󠁧󠁿 [UNIFIED_GATEWAY] Health check failed:', error);
    return false;
  }
}
