...
export async function fetchWithAuth(
 ...
 // CRITICAL: Add client ID header for API security
 ...
 console.log(`📡 [fetchWithAuth] ${restOptions.method || 'GET'} ${url}`, {
 ...
 try {
  ...
  console.error(`❌ [fetchWithAuth] Network error for ${url}:`, networkError);
  throw networkError;
 }

 // 🚨 ZOMBIE SESSION DETECTION
 if (response.status === 401 && !skipZombieDetection) {
  console.warn(`🧟 [fetchWithAuth] 401 Unauthorized from ${url}`, {
  ...
  if (hasAuthToken || hasUserInAuth) {
   console.error('🧟 Zombie session detected via 401 response!');
 ...
 if (!response.ok) {
  console.warn(`⚠️ [fetchWithAuth] ${response.status} ${response.statusText} from ${url}`);
 } else {
  console.log(`✅ [fetchWithAuth] ${response.status} from ${url}`);
 }
