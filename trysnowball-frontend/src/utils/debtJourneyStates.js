...
export const calculateDebtJourneyState = (debts = [], paymentHistory = [], projections = null) => {
  ...
  const activeDebts = debts.filter(debt => (debt.balance || 0) > 0);
  const hasDebts = activeDebts.length > 0;
  const hasEverHadDebt = debts.length > 0 || paymentHistory.length > 0;
  
  // Calculate progress percentage
  const totalOriginal = debts.reduce((sum, debt) => sum + (debt.originalAmount || debt.balance || 0), 0);
  const totalCurrent = activeDebts.reduce((sum, debt) => sum + (debt.balance || 0), 0);
...
export const getJourneyStateAction = (ctaAction, navigate) => {
  const actions = {
    ...
    pushFinish: () => navigate('/my-plan'),
    celebrate: () => navigate('/my-plan?celebrate=true'),
    explorePlan: () => navigate('/my-plan')
  };
  
  return actions[ctaAction] || (() => navigate('/my-plan'));
};
