...
const simulateSnowball = (debts, totalPayment) => {
  ...
  const totalDebt = snowballDebts.reduce((sum, debt) => sum + debt.balance, 0);
  ...
  for (let month = 1; month <= 120; month++) {
    ...
    for (let i = 0; i < snowballDebts.length; i++) {
      ...
      const actualPayment = Math.min(debt.balance, debt.minPayment);
      debt.balance = Math.max(0, debt.balance - actualPayment);
      
      // Deduct actual payment from available funds
      available -= debt.minPayment; // Always deduct full minimum from available
    }
    ...
    if (available > 0) {
      for (let i = 0; i < snowballDebts.length; i++) {
        const debt = snowballDebts[i];
        if (debt.balance > 0) {
          const extraPayment = Math.min(available, debt.balance);
