...
export const detectMilestone = (debts, previousDebts) => {
 ...
 const clearedDebts = debts.filter(debt => {
  ...
  return debt.amount_pennies === 0 && prevDebt && prevDebt.balance > 0;
 });
 ...
 const totalDebt = debts.reduce((sum, debt) => sum + (debt.amount_pennies || 0), 0);
 const prevTotalDebt = previousDebts.reduce((sum, debt) => sum + (debt.amount_pennies || 0), 0);
 ...
 const hasActiveDebts = debts.some(debt => debt.amount_pennies > 0);
 const hadActiveDebts = previousDebts.some(debt => debt.amount_pennies > 0);
