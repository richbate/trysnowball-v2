...
try { localDebtManager = require('../storage/localDebtManager').localDebtManager; } catch {}
...
async function getTotalsViaIDB(name) {
 ...
 db.close();
 const total = debts.reduce((s,d)=>s+(d.amount_pennies||0),0);
 const min = debts.reduce((s,d)=>s+(d.min_payment_pennies||0),0);
