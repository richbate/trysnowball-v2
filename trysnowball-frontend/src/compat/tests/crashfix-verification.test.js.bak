 ...
 describe('toDebtArray prevents "reduce is not a function" crashes', () => {
  ...
  crashScenarios.forEach(({ name, input }) => {
   it(`handles ${name} without crashing`, () => {
    ...
    expect(() => {
     result.reduce((sum, debt) => sum + (debt.amount_pennies || 0), 0);
    }).not.toThrow();
    
    expect(() => {
     result.filter(debt => debt.amount_pennies > 0);
    }).not.toThrow();
 ...
 describe('Array method safety after normalization', () => {
  it('supports chaining array methods safely', () => {
   ...
   corruptedInputs.forEach(input => {
    expect(() => {
     const result = toDebtArray(input)
      .filter(debt => debt.amount_pennies > 50)
      .map(debt => ({ ...debt, formatted: `$${debt.amount_pennies}` }))
      .reduce((sum, debt) => sum + debt.amount_pennies, 0);
 ...
 describe('Production-like scenarios', () => {
  it('handles empty state after clearing demo data', () => {
   ...
   emptyResponses.forEach(response => {
    ...
    const totalBalance = debts.reduce((sum, debt) => sum + (debt.amount_pennies || 0), 0);
    const totalMinPayments = debts.reduce((sum, debt) => sum + (debt.min_payment_pennies || 0), 0);
    const hasActiveDebts = debts.filter(debt => (debt.amount_pennies || 0) > 0).length > 0;
   ...
   malformedResponses.forEach(response => {
    expect(() => {
     ...
     const metrics = validDebts.reduce((acc, debt) => ({
      total: acc.total + (Number(debt.amount_pennies) || 0),
      count: acc.count + 1,
      avgBalance: 0
