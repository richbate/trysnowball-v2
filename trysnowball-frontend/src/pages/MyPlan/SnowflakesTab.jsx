...
export default function SnowflakesTab({ 
  ...
  // Use the same debt data source as other tabs
  ...
  const baseTimeline = useMemo(() => {
    ...
    return calculateTimeline(inputs.payoffStrategy, { ...inputs, debts, monthlyBudget });
  }, [calculateTimeline, inputs, debts, monthlyBudget]);
  ...
  }, [calculateTimeline, inputs, debts, monthlyBudget, snowflakesMap]);
  ...
  if (debts.length === 0) {
    return (
      <div className="text-center py-12">
        <Snowflake className="mx-auto h-12 w-12 text-gray-300 mb-4" />
        <h3 className="text-lg font-semibold text-gray-900 mb-2">No debts to add snowflakes to</h3>
        <p className="text-gray-600">Add some debts first to start making extra payments.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Snowflakes ❄️</h2>
          <p className="text-gray-600 mt-1">One-off extra payments to accelerate your debt freedom</p>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  className={`w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                    errors.debtId ? 'border-red-500' : 'border-gray-300'
                  }`}
                  required
                >
                ...
                {errors.debtId && (
                  <p className="text-sm text-red-600 mt-1">{errors.debtId}</p>
                )}
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className={`w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                    errors.monthIndex ? 'border-red-500' : 'border-gray-300'
                  }`}
                  required
                >
                ...
                {errors.monthIndex && (
                  <p className="text-sm text-red-600 mt-1">{errors.monthIndex}</p>
                )}
              </div>
            </div>
      ...
      {snowflakes.length === 0 ? (
        <div className="text-center py-12 bg-gray-50 rounded-lg">
          <Snowflake className="mx-auto h-12 w-12 text-gray-300 mb-4" />
          <h3 className="text-lg font-semibold text-gray-900 mb-2">No snowflakes yet</h3>
        ...
        <div className="space-y-4">
          {Object.keys(snowflakesByMonth)
            ...
            .map(monthIndex => (
              <div key={monthIndex} className="bg-white border rounded-lg p-4">
                ...
                <div className="space-y-2">
                  {snowflakesByMonth[monthIndex].map(snowflake => (
                    <div key={snowflake.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                      <div className="flex-1">
                        ...
                        {snowflake.note && (
                          <p className="text-sm text-gray-600 mt-1">{snowflake.note}</p>
                        )}
                      </div>
