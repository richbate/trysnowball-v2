...
export type Debt = {
  ...
  apr_bps: number;           // basis points (e.g., 1999 = 19.99%)
...
export type Payment = {
  ...
  principal_cents?: number;  // default = amount_cents
...
export async function savePayment(payment: Payment): Promise<void> {
  // Default principal_cents to amount_cents if not provided
  if (!payment.principal_cents) {
    payment.principal_cents = payment.amount_cents;
  }
...
export async function createMonthlySnapshot(userId: string): Promise<void> {
  ...
  const totalBalanceCents = debts.reduce((sum, debt) => sum + debt.balance_cents, 0);
