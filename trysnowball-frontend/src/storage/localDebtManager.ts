...
// - Stores money in cents, APR in basis points
  ...
  return await new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = () => {
      ...
      if (!pays.indexNames.contains('uniqUserDebtDateAmt'))
        pays.createIndex(
          'uniqUserDebtDateAmt',
          ['user_id', 'debt_id', 'occurred_at', 'amount_cents'],
          { unique: true }
        );
...
async function applyPaymentAtomic(params: {
  ...
  const amount_cents = params.amount_cents | 0;
  if (amount_cents <= 0) throw new Error('applyPayment: amount_cents must be > 0');
  ...
  await idbPromise<void>((resolve, reject) => {
    ...
    getReq.onsuccess = () => {
      ...
      debt.balance_cents = Math.max(0, (debt.balance_cents | 0) - amount_cents);
...
async function getTotalMinPaymentsInternal(userId: string): Promise<number> {
  const rows = await getDebtsForUser(userId);
  return rows.reduce((s, d) => s + (d.min_payment_cents | 0), 0);
}
...
export const localDebtManager = {
  ...
  async addDebt(input: {
    ...
    const row: DebtRow = {
      ...
      apr_bps: interestToBps(input.interest),
      min_payment_cents: poundsToCents(input.minPayment),
  ...
  > {
    ...
    return rows.map((d) => ({
      ...
      interest: d.apr_bps / 100,
      minPayment: centsToPounds(d.min_payment_cents),
      order: d.order_index,
    }));
  },
  ...
  async recordPayment(debtId: string, amountPounds: number, occurredAtISO?: string): Promise<string> {
    ...
    const id = await applyPaymentAtomic({
      user_id: USER_ID,
      debt_id: debtId,
      amount_cents: poundsToCents(amountPounds),
  ...
  async getPaymentHistory(): Promise<{ id: string; debtId: string; amount: number; occurred_at: string }[]> {
    ...
    return rows.map((p) => ({
      id: p.id,
      debtId: p.debt_id,
      amount: centsToPounds(p.amount_cents),
